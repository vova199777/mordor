...
#include "images/afk_icon/afk_icon.png.h"

RwRaster *afk_icon_load = nullptr;
...
CPlayerTags::CPlayerTags()
{
	...
	int w, h;
	unsigned char *imageData = stbi_load_from_memory(afk_icon_png, afk_icon_png_len, &w, &h, NULL, 4);
	...
	afk_icon_load = RwRasterCreate(wz, hz, d, flags);
	afk_icon_load = RwRasterSetFromImage(afk_icon_load, splashImage);
				...
				if(pPlayer && pPlayer->IsActive() && pPlayer->m_bShowNameTag)
				{
					...
					if(pPlayerPed->GetDistanceFromCamera() <= pNetGame->m_fNameTagDrawDistance)
					{
						...
						if(pNetGame->m_bNameTagLOS)
						{
							...
							dwHitEntityBubble = ScriptCommand(&get_line_of_sight, 
								VecPos.X, VecPos.Y, VecPos.Z,
								pCam->pos1x, pCam->pos1y, pCam->pos1z,
								1, 0, 0, 0, 0);								
						}
						...
						{
								sprintf(szNickBuf, BOEV("%s (%d)"), pPlayerPool->GetPlayerName(playerId), playerId);
						...
						if(!pNetGame->m_bNameTagLOS || dwHitEntityBubble)
						{
							DrawBubble(&VecPos,pPlayerPed->GetDistanceFromCamera(),playerId);
						}						
					}
				}
			}
...
void CPlayerTags::DrawBubble(VECTOR* vec, float fDist, PLAYERID playerId)
{
	VECTOR TagPos;
	...
	(( void (*)(VECTOR*, VECTOR*, float*, float*, bool, bool))(pack("0x54EEC0")+1))(&TagPos, &Out, 0, 0, 0, 0);
	...
	if(pBubble->fDistance[playerId] > Out.Z)
	{
		ImVec2 bPos = ImVec2(Out.X, Out.Y - 30); 
		pBubble->DrawAll(bPos, playerId);
	}
}

void CPlayerTags::Draw(VECTOR* vec, char* szName, uint32_t dwColor, 
	float fDist, float fHealth, float fArmour, bool bAfk, PLAYERID playerId)
{
	VECTOR TagPos;
	...
	if(Out.Z < 1.0f || Out.Z > 40.0f)
		return;

	// name (id)
	ImVec2 pos = ImVec2(Out.X, Out.Y);
	//sprintf(szName,"%s (%0.2f)",szName,Out.Z);
	...
	if(bAfk)
	{
		...
		ImGui::GetBackgroundDrawList()->AddImage((ImTextureID)afk_icon_load, a, b);
	}
}